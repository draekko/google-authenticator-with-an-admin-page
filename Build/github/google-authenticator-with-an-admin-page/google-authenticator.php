<?php
/*
Plugin Name: Google Authenticator With Admin Page
Plugin URI: http://henrik.schack.dk/google-authenticator-for-wordpress
Description: Two-Factor Authentication for WordPress using the Android/iPhone/Blackberry app as One Time Password generator.
Author: Henrik Schack, Modified Draekko
Version: 0.47
Author URI: http://henrik.schack.dk/
Compatibility: WordPress 3.9.1
Text Domain: google-authenticator-with-admin-page
Domain Path: /lang

Icon from http://www.iconarchive.com/show/mmii-flat-vol-4-icons-by-stalker018/google-authenticator-icon.html
...
function __construct() {
    self::$instance = $this;
	 ...
	 add_action( 'admin_menu', array( $this, 'ga_admin_create_menu' ) );
}
...
function profile_personal_options() {
	...
	// Use username@site.com as default description
	if ( '' == $GA_description ) {
		$current_user = wp_get_current_user();
		$host = parse_url(home_url());
		$domain = str_ireplace('www.', '', $host['host']);
		$GA_description = $current_user->user_login."@".$domain; 
	}
	...
	if ( $is_profile_page || IS_PROFILE_PAGE ) {
		if (trim( get_option('googleauthenticator_relaxedmode') ) != '') {
			echo "<tr>\n";
		...
		if ( trim( get_option('googleauthenticator_disable_qr_toggle') ) == '' ) {
	...
	if (trim( get_option('googleauthenticator_force_qr_display') ) != '') {
		echo "ShowQRCode();\n";
	}
...
* Create admin menu 
*/
function ga_admin_create_menu() {
    //create new top-level menu
    add_menu_page('Google Autheticator Site Settings', 'Authenticator', 'administrator',  __FILE__, array( &$this, 'ga_admin_settings_page' ), plugins_url('/images/icon.png', __FILE__));
...
function register_ga_settings() {
    ...
    register_setting( 'googleauthenticator-settings-group', 'googleauthenticator_code_issuer' );
    register_setting( 'googleauthenticator-settings-group', 'googleauthenticator_force_qr_display' );
    register_setting( 'googleauthenticator-settings-group', 'googleauthenticator_disable_qr_toggle' );
    register_setting( 'googleauthenticator-settings-group', 'googleauthenticator_relaxed_mode' );
}

/** 
* admin settings page 
*/
function ga_admin_settings_page() {

	$this->do_update_admin_settings();
	
	echo "<div class=\"wrap\">\n";
	echo "<h2>Google Authenticator Site Settings</h2>";

	$GA_force_display_qr =  trim ( get_option('googleauthenticator_force_qr_display') );
	$GA_disable_qr_toggle = trim ( get_option('googleauthenticator_disable_qr_toggle') );
	$GA_relaxed_mode = trim ( get_option('googleauthenticator_relaxed_mode') );
	$GA_code_issuer = trim( sanitize_text_field( get_option('googleauthenticator_code_issuer') ) );
	if (empty($GA_code_issuer)) {
		$GA_code_issuer = 'WordPress';
	}
	if (empty($GA_relaxed_mode)) {
		$GA_relaxed_mode = '';
	}
	if (empty($GA_disable_qr_toggle)) {
		$GA_disable_qr_toggle = '';
	}
	if (empty($GA_force_display_qr)) {
		$GA_force_display_qr = '';
	}
	
	//$postaction = plugins_url('settings.php', __FILE__);
   ...
   do_settings_sections( 'googleauthenticator-settings-group' ); ?>
    ...
    <table class="form-table">
        <tr valign="top">
        <th scope="row">Code Issuer</th>
        <td><input type="text" name="GA_code_issuer" id="GA_code_issuer" value="<?php echo $GA_code_issuer; ?>" /></td>
        </tr>
         
        <tr valign="top">
        <th scope="row">Force QR code display</th>
        <td><input type="checkbox" name="GA_force_display_qr" id="GA_force_display_qr" <?php $this->dochecked($GA_force_display_qr); ?> /></td>
        </tr>
        
        <tr valign="top">
        <th scope="row">Disable QR code toggle</th>
        <td><input type="checkbox" name="GA_disable_qr_toggle" id="GA_disable_qr_toggle" <?php $this->dochecked($GA_disable_qr_toggle); ?> /></td>
        </tr>
        
        <tr valign="top">
        <th scope="row">Disable relaxed mode</th>
        <td><input type="checkbox" name="GA_relaxed_mode" id="GA_relaxed_mode" <?php $this->dochecked($GA_relaxed_mode); ?> /></td>
        </tr>
    </table>
...
/**
* do checked for checkbox replacement
*/
function dochecked($status) {
...
function do_update_admin_settings() {
	if (isset($_POST['submit'])) {
		// Get _POST action
		$GA_code_issuer_saved = trim( sanitize_text_field( $_POST['GA_code_issuer'] ) );
		$GA_force_display_qr_saved = trim( sanitize_text_field( $_POST['GA_force_display_qr'] ) );
		$GA_disable_qr_toggle_saved = trim( sanitize_text_field( $_POST['GA_disable_qr_toggle'] ) );
		$GA_relaxed_mode_saved = trim( sanitize_text_field( $_POST['GA_relaxed_mode'] ) );
		
		// save options
		update_option('googleauthenticator_code_issuer', $GA_code_issuer_saved);
		update_option('googleauthenticator_force_qr_display', $GA_force_display_qr_saved);
		update_option('googleauthenticator_disable_qr_toggle', $GA_disable_qr_toggle_saved);
		update_option('googleauthenticator_relaxed_mode', $GA_relaxed_mode_saved);
	}
}
